...
void Dice::rollDice(int* maxRoll, int* numOfArmies) {
	...
	std::cout << "Roll from 1 to " << numDice << " dice (you have " << *numOfArmies << " armies)" << std::endl;
	cin >> temp;

	while (std::cin.fail() || temp < 1 || temp > numDice)
	{
		std::cout << "Error: Roll from 1 to " << numDice << " dice (you have " << *numOfArmies << " armies)" << std::endl;
		std::cin.clear();
